    ...
    private val postRepository: PostRepository
     ...
     suspend operator fun invoke(): Flow<Resource<List<PostModel>>> = flow {
        try {
            ...
            Log.d("get all posts use case response","${response.body()}")

            if (response.isSuccessful && response.body() != null ) {

                val postsData = response.body()
                Log.d("get all posts use case response posts data", postsData.toString())

                if(!postsData.isNullOrEmpty()){
                 val posts = postsData.map{it.toPostModel()}
                    Log.d("get all posts use case response posts ", posts.toString())
            ...
            e.localizedMessage?.let { Log.d("Get All PostsFlow Failed", it) }

            emit(Resource.Error("Server error: ${e.localizedMessage}"))
        } catch (e: IOException) {
            Log.d("Get All PostsFlow Failed","Failed to get posts")
